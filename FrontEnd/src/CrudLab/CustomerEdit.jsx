...
const CustomerEdit = () => {
    const { cutomersid } = useParams();

    useEffect(() => {
        fetch("https://localhost:7099/api/Products/" +  cutomersid)
            .then((res) => res.json())
            .then((resp) => {
                idchange(resp.id);
    ...
    const handlesubmit = (e) => {
      e.preventDefault();
      const  cutomersdata = { id, name, description, price, stock, imageURL, categoryName };
      ...
      fetch(`https://localhost:7099/api/Products/${productsid}`, {
        ...
        body: JSON.stringify( cutomersdata)
      })
        .then((res) => {
          if (res.ok) {
            alert('Saved successfully.');
            navigate('/admincutomers');
 ...
 <form  onSubmit={handlesubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
   ...
   <div className="flex items-center justify-between">
     ...
     <Link to="/admincutomers" className="bg-yellow-300 hover:bg-yellow-600 text-black font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Back</Link>
